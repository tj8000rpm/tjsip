...
// Copy form Golang original TextProto reader.go
// due to allowness compact form style against HTTP
...
func (r *Reader) ReadMIMEHeader() (textproto.MIMEHeader, error) {
	...
	for {
		...
		if len(key) == 1 {
			c := strings.IndexByte(key, 0)
			if 'A' <= c && c <= 'Z' {
				c -= toLower
			}
			key = canonicalMIMEHeaderKey(translateCompactForm[c])
		}
...
var translateCompactForm = map[int][]byte{
	'a': []byte("Accept-Contact"),      //[RFC3841]
	'b': []byte("Referred-By"),         //[RFC3892]
	'c': []byte("Content-Type"),        //[RFC3261]
	'd': []byte("Request-Disposition"), //[RFC3841]
	'e': []byte("Content-Encoding"),    //[RFC3261]
	'f': []byte("From"),                //[RFC3261]
	'i': []byte("Call-Id"),             //[RFC3261]
	'j': []byte("Reject-Contact"),      //[RFC3841]
	'k': []byte("Supported"),           //[RFC3261]
	'l': []byte("Content-Length"),      //[RFC3261]
	'm': []byte("Contact"),             //[RFC3261]
	'o': []byte("Event"),               //[RFC6665] [RFC6446]
	'r': []byte("Refer-To"),            //[RFC3515]
	's': []byte("Subject"),             //[RFC3261]
	't': []byte("To"),                  //[RFC3261]
	'u': []byte("Allow-Events"),        //[RFC6665]
	'v': []byte("Via"),                 //[RFC3261] [RFC7118]
	'x': []byte("Session-Expires"),     //[RFC4028]
	'y': []byte("Identity"),            //[RFC8224]
}
